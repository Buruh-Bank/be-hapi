    ...
    const preResponse = (request, h) => {
      ...
      if (response.isBoom) {
        ...
        const msg =
          response.data && response.data.msg
            ...
            : response.message;
        const data =
          response.data && response.data.data ? response.data.data : undefined;
        const err = withBasicError(code, msg, data);
        // Fix for F5
        if (
          response.output.statusCode === 500 ||
          response.output.statusCode === 503 ||
          response.output.statusCode === 504
        ) {
          statusCode = 400;
        }
      ...
      if (request.path != "/documentation" && request.path != "/status") {
        ...
        response.header(
          ...
          config.contentSecurityPolicyValue
        );
        response.header(
